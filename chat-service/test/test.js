...
describe('One user joins room', () => {  
    it('should return user joined interview', done => {    
        const client = io(socketUrl)

        client.emit('join', user1)
        client.on('receive', (res) => {
            res.message.should.equal('CS2101 has joined the interview')
...
describe('Two users join room', () => {  
    it('should return second user joined interview', done => {
        ...
        client1.on('receive', (res) => {
            if (res.message === "CS2102 has joined the interview") {
                res.message.should.equal('CS2102 has joined the interview')
                res.username.should.equal('ChatBot')
